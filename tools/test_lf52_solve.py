"""
Test solving lf52 Level 0 across 5 seeds using 4 peg jumps.
"""
import sys
...
def test_lf52_solve():
    ...
    for seed in range(5):
        ...
        for (fx, fy), (tx, ty) in moves:
            # Click from peg
            env.step(GameAction.ACTION6, data={"x": fx * 6 + 3, "y": fy * 6 + 3})
            # Click to destination arrow
            obs = env.step(GameAction.ACTION6, data={"x": tx * 6 + 3, "y": ty * 6 + 3})
            
        print(f"Seed {seed}: levels_completed={obs.levels_completed}")
        scores.append(obs.levels_completed)
        
    print(f"LF52 Multi-Seed Scores: {scores}")
