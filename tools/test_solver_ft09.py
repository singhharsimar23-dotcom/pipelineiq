...
def solve_ft09(env, max_steps: int = 2000) -> dict:
    """
    Isolated ft09 solver implementing Steps 1-4 without hardcoded coordinates/colors.
    """
    total_steps = 0
    obs_current = env.step(GameAction.RESET, data={})
    total_steps += 1

    grid = get_2d_grid(obs_current)
    bg = get_background_color(grid)
    initial_levels_completed = getattr(obs_current, "levels_completed", 0)
    ...
    # Find spatial cluster occupying smallest bounding box containing >= 6 components
    best_cluster = None
    min_bbox_area = float('inf')

    if len(candidates) >= 6:
        for c0 in candidates:
            sorted_cands = sorted(candidates, key=lambda c: (c['cx'] - c0['cx'])**2 + (c['cy'] - c0['cy'])**2)
            for k_sub in range(6, min(10, len(candidates) + 1)):
                subset = sorted_cands[:k_sub]
                min_r = min(c['min_r'] for c in subset)
                max_r = max(c['max_r'] for c in subset)
                min_c = min(c['min_c'] for c in subset)
                max_c = max(c['max_c'] for c in subset)
                bbox_area = (max_r - min_r + 1) * (max_c - min_c + 1)
                if bbox_area < min_bbox_area:
                    min_bbox_area = bbox_area
                    best_cluster = list(subset)
    else:
        best_cluster = list(candidates)

    if best_cluster is None:
        best_cluster = list(candidates)

    # Sort buttons in standard raster order (cy, cx)
    best_cluster.sort(key=lambda b: (b['cy'], b['cx']))
    ...
    for btn in best_cluster:
        bx, by = btn['cx'], btn['cy']
        f_old = get_2d_grid(obs_current)
        c_before = int(f_old[by, bx])

        obs_current = env.step(GameAction.ACTION6, data={"x": int(bx), "y": int(by)})
        total_steps += 1
        f_new = get_2d_grid(obs_current)
        c_after = int(f_new[by, bx])

        delta = int(np.sum(f_new != f_old))
        if delta > 0:
        ...
        # Issue RESET and wait for level reset
        obs_current = env.step(GameAction.RESET, data={})
        total_steps += 1
    ...
    obs_current = env.step(GameAction.RESET, data={})
    total_steps += 1
    f_current = get_2d_grid(obs_current)
    ...
    for k in range(0, n_responsive + 1):
        for combo in combinations(responsive_buttons, k):
            ...
            if obs_current is not None:
                st = getattr(obs_current, "state", None)
                lvl_done = getattr(obs_current, "levels_completed", 0)
...
def run_full_verification():
    ...
    for seed in SEEDS:
        ...
        sc_id = arc.create_scorecard(tags=[f"ft09_verif_seed_{seed}"])
